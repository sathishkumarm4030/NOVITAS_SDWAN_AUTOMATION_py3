...
class VersaLib:
    ...
    def __init__(self, device_name, **kwargs):
        ...
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        if 'topofile' in self.__dict__:
            if ".csv" in self.topofile:
                ...
                for k, v in list(csv_dict[self.Device_name].items()):
                ...
                for k, v in list(device_dict.items()):
                    ...
                    if v == None:
                        continue
                    exec("self." + k + '=v')
            if "POD" in self.__dict__:
                self.ctlr_dict = POD[self.POD]['ctlr_dict']
                self.ctlr_list = POD[self.POD]['ctlr_list']
                self.staging_servers_dict = POD[self.POD]['staging_servers_dict']
                self.LCC_dict = POD[self.POD]['LCC_dict']
                self.gw_dict = POD[self.POD]['gw_dict']
                self.gw_list = POD[self.POD]['gw_list']
                self.SOLUTIONS_list = SOLUTIONS_list
                self.log_collector = POD[self.POD]['log_collector']
                self.TRACK_MANAGEMENT_SUBNET = POD[self.POD]['TRACK_MANAGEMENT_SUBNET']
                self.RR_SERVER = POD[self.POD]['RR_SERVER']
                self.RR_Clients = POD[self.POD]['RR_Clients']
                self.MGMT_NW_SBNT = POD[self.POD]['MGMT_NW_SBNT']
                self.VNF_IPADDRESS = POD[self.POD]['VNF_IPADDRESS']
        ...
        if 'device_type' in self.__dict__:
            if self.device_type == 'versa_director':
                ...
                if "POD" in self.__dict__:
                    self.vddata_dict = POD[self.POD]['VD1']
    ...
    def create_cpe_data(self):
        ...
        # elif self.WAN2_LIB == True or self.WAN1_LIB == True:
        #     self.FW_PROFILE = "COMMON-SFW-TEMPLATE"
        # else:
        #     self.FW_PROFILE = "GENERIC-SFW-TEMPLATE"

        if "SINGLE-CPE-HYBRID" in self.Solution_type:
            if "LIB1" in self.__dict__ and self.LIB1 == True:
                self.FW_PROFILE = "COMMON-SFW-TEMPLATE"
            else:
                self.FW_PROFILE = "GENERIC-SFW-TEMPLATE"
        ...
        if "SINGLE-CPE-DUAL-INTERNET" == self.Solution_type:
            if ("LIB1" in self.__dict__ and self.LIB1 == "ACTIVE") or (
                    "LIB2" in self.__dict__ and self.LIB2 == "ACTIVE"):
                self.FW_PROFILE = "COMMON-SFW-TEMPLATE"
            else:
                self.FW_PROFILE = "GENERIC-SFW-TEMPLATE"
        ...
        self.WC1_NAME = self.ctlr_dict[self.NODE][0]
        self.WC2_NAME = self.ctlr_dict[self.NODE][1]
        self.GW1_NAME = self.gw_dict[self.NODE][0]
        self.GW2_NAME = self.gw_dict[self.NODE][1]
        ...
        self.MGMT_NW_SBNT = self.MGMT_NW_SBNT
        self.VNF_IPADDRESS1 = self.VNF_IPADDRESS[0]
        self.VNF_IPADDRESS2 = self.VNF_IPADDRESS[1]
    ...
    def set_network_items(self, START_LAN_IP_SUBNET):
        ...
        for i in range(1, int(self.NO_OF_VRFS) + 1):
            ...
            if 'ORG_ID' in self.__dict__:
                self.lan[i]['bgp_instance_id'] = str(self.ORG_ID * 10 + 120 + 3000 + 12 + i - 1)
    ...
    def shell_login(self, **kwargs):
        ...
        self.main_logger.debug(self.shell_nc.send_command_expect('sudo bash', expect_string='password', cmd_verify=False))
        self.main_logger.debug(self.shell_nc.send_command_expect(self.password, expect_string='\~|#|\$', cmd_verify=False))
        self.main_logger.debug(self.shell_nc.send_command_expect('exit', expect_string='\$|#', cmd_verify=False))
    ...
    def cross_login(self):
        ...
        time.sleep(2)
        rs = self.cnc.send_command_expect("set idle-timeout 0", expect_string='>', strip_prompt=False,
                                          strip_command=False)
        print(rs)
        return self.cnc
    ...
    def get_task_result(self, taskid):
        ...
        if task_result == 'FAILED':
            if isinstance(data1['versa-tasks.task']['versa-tasks.errormessages'], dict) \
                    and 'versa-tasks.errormessage' in data1['versa-tasks.task']['versa-tasks.errormessages']:
                if isinstance(data1['versa-tasks.task']['versa-tasks.errormessages']['versa-tasks.errormessage'], list) \
                    ...
                    try:
                        if 'versa-tasks.error-message' in data1['versa-tasks.task']['versa-tasks.errormessages']['versa-tasks.errormessage'][
                            0] and \
    ...
    def get_device_info(self):
        ...
        for i in data1['versanms.ApplianceStatusResult']['appliances']:
            if i['name'] == self.Device_name and i['ipAddress'] == self.ESP_IP:
                ...
                #20.2.2 onwards controller info removed
                # try:
                #     if i['controllers'] != "":
    ...
    def device_config_commands(self, nc_handler, cmds):
        ...
        for cmd in cmds.split("\n"):
            self.main_logger.debug(
                nc_handler.send_command_expect(cmd, expect_string='%', strip_prompt=False, strip_command=False))
        self.main_logger.debug(nc_handler.send_command_expect('commit and-quit', expect_string='>', strip_prompt=False,
                                                              strip_command=False))
    ...
    def device_config_commands_with_return(self, nc, cmds):
        ...
        for cmd in cmds.split("\n"):
            self.main_logger.debug(
                nc_handler.send_command_expect(cmd, expect_string='%', strip_prompt=False, strip_command=False))
        commit_result = nc_handler.send_command_expect('commit and-quit', expect_string='>', strip_prompt=False,
                                                       strip_command=False)
    ...
    def device_config_commands_wo_split(self, nc_handler, cmds):
        ...
        return nc_handler.send_config_set(config_commands=cmds, strip_prompt=False, strip_command=False, \
                                          max_loops=5000, delay_factor=0.0001, exit_config_mode=False, cmd_verify=False)
    ...
    def device_request_commands(self, nc_handler, cmds):
        ...
        for cmd in cmds.split("\n"):
            self.main_logger.info(
                nc_handler.send_command_expect(cmd, expect_string='>', strip_prompt=False, strip_command=False,
                                               max_loops=1000, delay_factor=0.0001))
        return "PASS"

    def linux_device_config_commands(self, nc_handler, cmds, expect_string="\$"):
        for cmd in cmds.split("\n"):
            # print cmd
            self.main_logger.info(nc_handler.send_command_expect(cmd, expect_string=expect_string, strip_prompt=False,
                                                                 strip_command=False))
    ...
    def cpe_onboard_call(self):
        ...
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('vsh allow-cli', expect_string='password', strip_prompt=False,
                                                strip_command=False, cmd_verify=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect(self.password, expect_string='CLI now allowed', strip_prompt=False,
                                                strip_command=False, cmd_verify=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('cli', expect_string='>', strip_prompt=False, strip_command=False, cmd_verify=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('request erase running-config', expect_string='yes', strip_prompt=False,
                                                strip_command=False, cmd_verify=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('yes', expect_string='\$|#', strip_prompt=False, strip_command=False, cmd_verify=False))
        ...
        self.main_logger.debug(cpe_shell_login.send_command_expect('vsh status', expect_string='\$|#', cmd_verify=False))
        self.main_logger.debug(cpe_shell_login.send_command_expect('vsh show-serialnum', expect_string='\$|#', cmd_verify=False))
        ...
        self.main_logger.debug(cpe_shell_login.send_command_expect(self.Staging_command_template, expect_string='\$|#', cmd_verify=False))
        time.sleep(20)
    ...
    def check_org_for_controller(self, controller):
        ...
        if 'versanms.sdwan-org-workflow' in data1:
    ...
    def create_and_deploy_poststaging_template(self):
        ...
        if "DUAL-CPE" in self.Solution_type:
            ...
            assc_body = '[{"organization":"' + self.ORG_NAME + '","serviceTemplate":"' + self.FW_PROFILE + str(
                self.NO_OF_VRFS) + '"}]'
            ...
            if 'FAIL' in fw_association:
                self.main_logger.info(
                    ">>>>>>>>>> FW PROFILE ASSOCIATION WITH POST STAGING TEMPLATE FAILED. <<<<<<<<<<<")
                exit()
            else:
                self.main_logger.info(
                    ">>>>>>>>>> FW PROFILE ASSOCIATION WITH POST STAGING TEMPLATE PASSED. <<<<<<<<<<<")
            time.sleep(5)
            ps_deploy_result = self.post_operation(
                template_url + "/deploy/" + self.PS_TEMPLATE_NAME + "?verifyDiff=true", headers3,
                self.ps_template_body)
    ...
    def ping_ipv6(self, dest_ip, src_ip, lan, **kwargs):
        cmd = "ping " + str(dest_ip) + " routing-instance " + lan + "-VRF" + " source " + str(src_ip)
        print(cmd)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return str(" 0% packet loss" in output)
    ...
    def show_interface_brief_lan(self, intf_name, vlan):
        cmd = "show interfaces brief " + str(intf_name) + "." + str(vlan)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def show_ipv6_config_intf(self, inet6_add):
        cmd = "show configuration | display set | match " + str(inet6_add)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output
        # Do modify for python3
        group_name = LAN - EBGP

    def show_conf_bgp(self, group_name):
        cmd = "show configuration | display set | match " + str(group_name)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_conf_bgp_redis(self, community_id):
        cmd = "show configuration | display set | match " + str(community_id)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def get_bgp_redis(self, Customer_ORG_Name, loopback_ip):
        cmd = "show route table l3vpn.ipv4.unicast routing-instance " + str(
            Customer_ORG_Name) + " receive-protocol bgp " + str(loopback_ip) + " | nomore"
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_bgp_nbr(self, lan, nbr_address):
        cmd = "show bgp neighbor brief LAN" + lan + "-VRF neighbor-ip " + str(nbr_address)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_conf_ospf(self, Protocol):
        cmd = "show configuration | display set | match " + str(Protocol)
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print (output)
        return output

    def show_ipv6_route(self, lan):
        cmd = "show route table ipv6.unicast routing-instance LAN" + lan + "-VRF | nomore"
        output = self.cnc.send_command_expect(cmd, expect_string="cli>", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_ipv6_conf_sroute(self, lan):
        cmd = "show configuration routing-instances LAN" + lan + "-VRF " + "routing-options " + "static | display set | nomore "
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_ipv6_sroute(self, lan):
        cmd = "show route routing-instance LAN" + lan + "-VRF | nomore"
        output = self.cnc.send_command_expect(cmd, expect_string="cli>", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_ipv6_conf_bgproute(self, lan):
        cmd = "show configuration routing-instances LAN" + lan + "-VRF " + "protocols " + "bgp | display set | nomore"
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        print(output)
        return output

    def show_config_display_set(self, policy='Default-Policy', **kwargs):
        cmd = "show orgs org-services " + self.ORG_NAME + " class-of-service qos-policies " + policy + " rules " + \
              " LAN1-VRF-Premium | display set "
        print(cmd)
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def get_bgp_nbr_status(self, nbr_ip, instance=""):
        if instance == "":
            instance = self.ORG_NAME + "-Control-VR"
        cmd = "show bgp neighbor org " + self.ORG_NAME + " brief " + instance + " | match " + nbr_ip
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def get_ospf_nbr_status(self):
        cmd = "show ospf neighbor brief"
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

#VD cases

    def get_sys_pkg_info(self):
        cmd = "show system package-info"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def get_sec_oss_info(self):
        cmd = "show security osspack info"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def show_conf_nms(self):
        cmd = "show configuration nms provider spack"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def req_vnmsha_act_status(self):
        cmd = "request vnmsha actions status"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def req_postgres_details(self):
        cmd = "request vnmsha actions get-vnmsha-postgres-details"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)        
        return output

    def fetch_peer_vnmsha_details(self):
        cmd = "request vnmsha actions get-vnmsha-details fetch-peer-vnmsha-details true"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)        
        return output

    def req_sys_pkg_list(self):
        cmd = "request system package list"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)        
        return output

    def req_sys_upgrade_img(self,upgrade_img):
        cmd = "request system package upgrade" + str(upgrade_img)
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)       
        return output
    ...
    def log_moniter(self):
        cmd = "shell"
        output = self.nc.send_command_expect(cmd, expect_string="#", strip_prompt=False, strip_command=False)
        cmd = "tail -f /var/log/vnms/upgrade.log"
        output = self.nc.send_command_expect(cmd, expect_string="#", strip_prompt=False, strip_command=False)
        return output
...
#mos cases
    def config_rtm(self,Device_Name,ORG_NAME):
        cmd = "configure"
        output = self.nc.send_command_expect(cmd, expect_string="%", strip_prompt=False, strip_command=False)
        cmd = "set devices device " + str(Device_Name) + " config orgs org-services " + str(ORG_NAME) + " application-identification realtime-flow-quality monitoring enabled" 
        output = self.nc.send_command_expect(cmd, expect_string="%", strip_prompt=False, strip_command=False)
        cmd = "commit"
        output = self.nc.send_command_expect(cmd, expect_string="%", strip_prompt=False, strip_command=False)
        return output

    def show_conf_rtm(self):
        cmd = "show configuration orgs org-services " + self.ORG_NAME + " application-identification realtime-flow-quality monitoring"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def show_sip_rtp(self):
        cmd = "show orgs org " + self.ORG_NAME + " sessions sdwan brief | tab"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output

    def show_mos_score(self):
        cmd = "show orgs org " + self.ORG_NAME + " sessions extensive | sort-by mos-score | select codec | select application | select mos-score"
        output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
        return output
    ...
    def req_clr_stats_cos_qos_plcy_all(self):
        ...
        output = self.cnc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False, cmd_verify=False)
        return output
    ...
    def request_ping(self, net_connect, cpe):
        ...
        output = net_connect.send_command_expect(cmd, strip_prompt=False, strip_command=False, expect_string=">", cmd_verify=False)
    ...
    def request_connect(self, net_connect, cpe):
        ...
        self.main_logger.info("CMD>> : " + cmd)
        output = net_connect.send_command_expect(cmd, strip_prompt=False, strip_command=False, cmd_verify=False)
        self.main_logger.info(output)
    ...
    def request_sync_from_cpe(self, net_connect, cpe):
        ...
        self.main_logger.info("CMD>> : " + cmd)
        output = net_connect.send_command_expect(cmd, strip_prompt=False, strip_command=False, max_loops=5000, cmd_verify=False)
        self.main_logger.info(output)
        return str(" result true" in output)
    ...
    def config_function(self, nc, config_for, csv_file, template_file, type=""):
        ...
        for idx, row in csv_data_read.iterrows():
            ...
            if config_for == "bgp_grt":
                if type == "devices":
                    check_state_result = self.check_device_status(nc, dev_dict['NAME'])
                    if check_state_result != "PASS":
                        res_check += ".After Commit, " + check_state_result
                    checkbgp_nbr_state = self.check_dev_bgp_nbr(nc, dev_dict['NAME'], dev_dict['ORG_NAME'])
                    if 'fail' in checkbgp_nbr_state:
                        res_check += ".After Commit, " + checkbgp_nbr_state
            result_dict[dev_dict['NAME']] = res_check
            # main_logger.info(result_dict)
            self.main_logger.info("CONFIG_RESULT:")
            for k, v in list(result_dict.items()):
    ...
    def Do_commit_and_check_commit(self, nc, result):
        ...
        commit_result = nc.send_command_expect("commit", \
                                               expect_string='%', \
                                               strip_prompt=False, strip_command=False, max_loops=5000, cmd_verify=False)
        self.main_logger.debug(commit_result)
        ex_result = nc.send_command_expect("exit no-confirm", expect_string=">", strip_prompt=False,
                                           strip_command=False, max_loops=5000, cmd_verify=False)
    ...
    def check_dev_bgp_nbr(self, nc, device_name, org_name):
        new_device_status_dict = {}
        shw_dev_bgp_nbr_br = "show devices device " + device_name + " live-status bgp neighbor brief " + \
                             org_name + "-Control-VR | tab"
        self.main_logger.info("CMD>> : " + shw_dev_bgp_nbr_br)
        output = nc.send_command_expect(shw_dev_bgp_nbr_br, strip_prompt=False, strip_command=False, cmd_verify=False)
        nbrs = re.findall("Established", output, re.M)
        self.main_logger.info(output)
        if len(nbrs) == 2:
            return "BGP nbr check passed.BGP established nbr  count:" + str(len(nbrs))
        elif len(nbrs) < 2:
            return "BGP nbr check failed. BGP established nbr  count:" + str(len(nbrs))
    ...
    def check_functions(self, nc, csv_data_read, config_for, csv_file, template_file, type="", org_name="", **kwargs):
        ...
        for idx, row in csv_data_read.iterrows():
            ...
            if "WC" in dev_dict['NAME']:
                pass
            if dev_dict['NAME'] not in device_status_dict:
            ...
            if "WC" not in dev_dict['NAME']:
                if dev_dict['NAME'] in device_status_dict:
                ...
                shw_dev_bgp_nbr_br = "show devices device " + dev_dict['NAME'] + " live-status bgp neighbor brief " + \
                                     dev_dict['ORG_NAME'] + "-Control-VR | tab"
                self.main_logger.info("CMD>> : " + shw_dev_bgp_nbr_br)
                output = nc.send_command_expect(shw_dev_bgp_nbr_br, strip_prompt=False, strip_command=False)
                nbrs = re.findall("Established", output, re.M)
                if len(nbrs) == 2:
                    new_device_status_dict[dev_dict['NAME']] = "BGP nbr check passed.BGP established nbr  count:" + str(
                        len(nbrs))
                elif len(nbrs) < 2:
                    new_device_status_dict[
                        dev_dict['NAME']] = "BGP nbr check failed. BGP established nbr  count:" + str(len(nbrs))
                self.main_logger.info(output)
    ...
    def generic_config_function(self, nc, config_for, csv_file, template_file, type="", org_name="", **kwargs):
        ...
        template = curr_env.get_template(template_file)
        # if type != "ps":
        #     csv_data_read = csv_data_read.loc[csv_data_read['NAME'] == org_name]
        result_dict = {}
        device_cmds = ""
        if type == "ps":
            for idx, row in csv_data_read.iterrows():
                ...
                if config_for == "ipsec_ike_transform":
                    # if dev_dict['ORG_NAME'] == org_name:
                    device_cmds += template.render(dev_dict, IKE_TRANSFORM=ike_tranform,
                                                   IPSEC_TRANSFORM=ipsec_transform) + "\n"
            ...
            for k, v in list(self.device_status_dict1.items()):
            ...
            self.write_check_result_from_dict(self.device_status_dict1, org_name,
                                              "PRE_IPSEC_MODIFICATION_RESULT_" + csv_file)
            if "PASS" == self.search_a_value(list(self.device_status_dict1.values()), "fail"):
            ...
            if config_for == "ipsec_ike_transform":
                ...
                for idx, row in csv_data_read.iterrows():
                    ...
                    if "WC" not in dev_dict['NAME'] and "GW" not in dev_dict['NAME']:
                        ...
                        req_create_snap_cmd += req_create_snap_temp.render(dev_dict,
    ...
    def get_device_list_from_vd_using_rest_single_cpe(self, cpe_name):
        global batch, devices_list
        data1 = self.get_operation(appliance_url, headers3)
        count, day, batch = 1, 1, 1
        # print data1
        for i in data1['versanms.ApplianceStatusResult']['appliances']:
            ...
            if cpe_name == i['name']:
                ...
                #     print(i['name'])
                #     print("Hardware Info NIL")
                if i['last-updated-time'] != "":
                    device_list.append(i['last-updated-time'])
                else:
                    i['last-updated-time'] = "NULL"
                    device_list.append(i['last-updated-time'])
                return device_list
    ...
    def Create_Node_Data(self, node_dev, node_type, org_name="", org_id="", wan=""):
        ...
        for idx, row in csv_data_read.iterrows():
            ...
            for k, v in list(node_device_data.items()):
                if isinstance(v, str):
                    if "temporgname" in v:
            ...
            if node_type == "WC":
                node_device_data["VNF_GWS"] = self.gw_dict[node_device_data['NODE']]
                if node_dev in self.RR_Clients:
                    node_device_data["RR_CLIENTS"] = self.RR_Clients[node_dev]
                else:
                    for k, v in list(self.RR_Clients.items()):
                        if node_dev in self.RR_Clients[k]:
                            if k in self.RR_SERVER:
                                node_device_data["RR_SERVER"] = k
            if node_type == "GW":
                node_device_data["WC_list"] = self.ctlr_dict[node_device_data['NODE']]
            if "NODE" in self.__dict__:
                node_device_data["LCC"] = self.LCC_dict[self.NODE]
            else:
                node_device_data["LCC"] = self.LCC_dict[node_device_data['NODE']]
            if node_type == "SS":
                self.ndb[node_type] = {}
    ...
    def rest_add_sroute(self, lan_number, ip_prefix, next_hop, preference, **kwargs):
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        temp_dict = {}
        self.main_logger.info("\nCREATE static route in LAN vrf\n")
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        url1 = routing_instance_url.replace("temporgname", self.ORG_NAME)
        url1_mod = url1.replace("tempdevicename", self.Device_name)
        url_final = url1_mod + "LAN" + str(lan_number) + "-VRF/routing-options/static/route/"
        curr_env = Environment(loader=curr_file_loader)
        template1 = curr_env.get_template("static_route.j2")
        temp_dict['ip_prefix'] = ip_prefix
        temp_dict['next_hop'] = next_hop
        temp_dict['preference'] = preference
        temp_dict['interface'] = self.LAN_INTF + "." + str(self.START_VLAN)
        template_body = template1.render(temp_dict)
        print(template_body)
        result = self.post_operation(url_final, headers3, template_body)
        self.main_logger.info("\n" + result)
        if 'FAIL' in result:
            self.main_logger.info(">>>>>>>>>> CREATE static route FAILED. <<<<<<<<<<<")
            return "FAIL"
        else:
            self.main_logger.info(">>>>>>>>>> CREATE static route PASSED. <<<<<<<<<<<")
            return "PASS"
        time.sleep(5)

    def rest_del_sroute(self, lan_number, ip_prefix, next_hop, **kwargs):
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        temp_dict = {}
        self.main_logger.info("\nDELETE static route in LAN vrf\n")
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        url1 = routing_instance_url.replace("temporgname", self.ORG_NAME)
        url1_mod = url1.replace("tempdevicename", self.Device_name)
        url_final = url1_mod + "LAN" + str(lan_number) + \
                    "-VRF/routing-options/static/route/rti-static-route-list/" + '"' + ip_prefix + \
                    "," + next_hop + "," + self.LAN_INTF + "." + str(self.START_VLAN) + '"'
        curr_env = Environment(loader=curr_file_loader)
        result = self.delete_operation(url_final, headers3)
        self.main_logger.info("\n" + result)
        if 'FAIL' in result:
            self.main_logger.info(">>>>>>>>>> CREATE static route FAILED. <<<<<<<<<<<")
            return "FAIL"
    ...
    def rest_ha_fail_vd1(self, device_type, mgmt_ip, stdby_mgmt_ip, **kwargs):
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        temp_dict = {}
        self.main_logger.info("\nCREATE VD1 HA failover\n")
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps")
        #url1 = routing_instance_url.replace("temporgname", self.ORG_NAME)
        #url1_mod = url1.replace("tempdevicename", self.Device_name)
        url_final = "administration/system/high_availability"
        curr_env = Environment(loader=curr_file_loader)
        template1 = curr_env.get_template("vd1_ha.j2")
        temp_dict['device_type'] = device_type
        temp_dict['mgmt_ip'] = mgmt_ip
        temp_dict['stdby_mgmt_ip'] = stdby_mgmt_ip
        template_body = template1.render(temp_dict)
        print(template_body)
        result = self.post_operation(url_final, headers3, template_body)
        self.main_logger.info("\n" + result)
    ...
    def rest_ha_fail_vd2(self, device_type, mgmt_ip, stdby_mgmt_ip, **kwargs):
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        temp_dict = {}
        self.main_logger.info("\nCREATE VD2 HA failover\n")
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps")
        #url1 = routing_instance_url.replace("temporgname", self.ORG_NAME)
        #url1_mod = url1.replace("tempdevicename", self.Device_name)
        url_final = "administration/system/high_availability"
        curr_env = Environment(loader=curr_file_loader)
        template1 = curr_env.get_template("vd2_ha.j2")
        temp_dict['device_type'] = device_type
        temp_dict['mgmt_ip'] = mgmt_ip
        temp_dict['stdby_mgmt_ip'] = stdby_mgmt_ip
        template_body = template1.render(temp_dict)
    ...
    def config_lan_ebgp(self, Device_name, lan_number, bgp_instance_id, local_address, peer_as, password, nbr_address,
                        **kwargs):
        temp_dict = {}
        if kwargs is not None:
        ...
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("config_lan_ebgp.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['lan_number'] = lan_number
        temp_dict['bgp_instance_id'] = bgp_instance_id
        temp_dict['local_address'] = local_address
        temp_dict['peer_as'] = peer_as
        temp_dict['password'] = password
        temp_dict['nbr_address'] = nbr_address
        cmds = template.render(temp_dict)
        print(cmds)
        result = self.commit_config_commands(self.nc, cmds)
        return self.check_commit_result(result)

    def config_ipv6_lan_ebgp(self, Device_name, lan_number, bgp_instance_id, local_address, peer_as, password,
                             nbr_address, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
        ...
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("config_lan_ebgp_ipv6.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['lan_number'] = lan_number
        temp_dict['bgp_instance_id'] = bgp_instance_id
        temp_dict['local_address'] = local_address
    ...
    def delete_lan_ebgp(self, Device_name, lan_number, bgp_instance_id, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("delete_lan_ebgp.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['lan_number'] = lan_number
        temp_dict['bgp_instance_id'] = bgp_instance_id
        cmds = template.render(temp_dict)
    ...
    def delete_ospf(self, Device_name, lan_number, ospf_instance_id, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
    ...
    def config_bgp_redis(self, Device_name, Customer_ORG_Name, Customer_ORG_ID, sdwan_policy_name, lan_number, redis_policy_name, community_id, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("config_bgp_redis.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['Customer_ORG_Name'] = Customer_ORG_Name
        temp_dict['Customer_ORG_ID'] = Customer_ORG_ID
        temp_dict['sdwan_policy_name'] = sdwan_policy_name
        temp_dict['lan_number'] = lan_number
        temp_dict['redis_policy_name'] = redis_policy_name
    ...
    def delete_bgp_redis(self, Device_name, lan_number, redis_policy_name, Customer_ORG_Name, bgp_instance_id,
                         sdwan_policy_name, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("delete_bgp_redis.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['lan_number'] = lan_number
        temp_dict['redis_policy_name'] = redis_policy_name
        temp_dict['Customer_ORG_Name'] = Customer_ORG_Name
    ...
    def config_bgp_reject(self, Device_name, Customer_ORG_Name, Customer_ORG_ID, sdwan_policy_name, community_id,
                          **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
        ...
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("config_bgp_reject.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['Customer_ORG_Name'] = Customer_ORG_Name
        temp_dict['Customer_ORG_ID'] = Customer_ORG_ID
        temp_dict['sdwan_policy_name'] = sdwan_policy_name
    ...
    def config_ospf(self, Device_name, lan_number, ospf_instance_id, ospf_router_id, ospf_area_id, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
        curr_file_loader = FileSystemLoader(curr_file_dir + "/libraries/J2_temps/ROUTING_INSTANCE")
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("enable_ospf.j2")
        temp_dict['Device_name'] = Device_name
        temp_dict['lan_number'] = lan_number
        temp_dict['ospf_instance_id'] = ospf_instance_id
        temp_dict['ospf_router_id'] = ospf_router_id
    ...
    def ospf_auth(self, Device_name, lan_number, ospf_instance_id, ospf_router_id, ospf_area_id, password, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
    ...
    def ospf_md_auth(self, Device_name, lan_number, ospf_instance_id, ospf_router_id, ospf_area_id, password, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
    ...
    def upload_block_page_files(self, name, org_name_uuid, **kwargs):
        if kwargs is not None:
            for k, v in list(kwargs.items()): exec("self." + k + '=v')
        self.main_logger.info("\nUpload file\n")
        block_page_upload_url = '/vnms/captiveportal/custompages/appliance?pagename=Versa-Block-Page.zip&actiontype=block&appliance=tempdevicename&org-uuid=902ca3e6-4fc6-4afa-a6b8-e4e23ab3e59f'
        obj_url_mod = block_page_upload_url.replace("temporgname", org_name_uuid)
        obj_url_mod = obj_url_mod.replace("tempdevicename", name)
        template_body = {}
        print(template_body)
        obj_creation_result = self.post_operation(obj_url_mod, headers3, template_body)
        self.main_logger.info("\n" + obj_creation_result)
        if 'FAIL' in obj_creation_result:
            self.main_logger.info(">>>>>>>>>> ADDRESS OBJECT CREATION FAILED. <<<<<<<<<<<")
            return "FAIL"
        else:
            self.main_logger.info(">>>>>>>>>> ADDRESS OBJECT CREATION PASSED. <<<<<<<<<<<")
    ...
    def create_policy_rule_ipv6(self, name, fwd_profile_name, **kwargs):
        temp_dict = {}
        # if kwargs is not None:
        #     for k, v in kwargs.iteritems(): exec("self."+ k+'=v')
        if kwargs is not None:
        ...
        policy_rule_create_url_mod = policy_rule_create_url_mod.replace("tempdevicename", self.Device_name)
        curr_env = Environment(loader=curr_file_loader)
        template = curr_env.get_template("RULE_Creation_ipv6.j2")
        rule_dict = {'name': name, 'fwd_profile_name': fwd_profile_name}
        temp_dict['name'] = name
        temp_dict['fwd_profile_name'] = fwd_profile_name
        template_body = template.render(temp_dict)
    ...
    def Create_Org(self, org_name, org_id, WC_list, no_of_vrfs):
        """This functions used for org creation in versa director

        Args:
            org_name (str): org name
            org_id (int): org id
            WC_list (list): list of Wancontrollers for this Org
            no_of_vrfs (int): number of vrfs  in this Org


        Returns:
            str: PASS or "FAIL: " + task_result
        """
    ...
    def Config_Node_Devices(self, device_name, node_type, nodes, action="set", **kwargs):
        ...
        if org_data['DEVICE_TYPE'] == "WC":
            ...
            if 'RR_SERVER' in org_data:
                if org_data['RR_SERVER'] != "":
                    if org_data['RR_SERVER'] in self.ndb:
        ...
        if org_data['DEVICE_TYPE'] == "GW":
            ...
            org_data['PAIRED_TVI_INTERFACE_SET'] = ""
            org_data['MPLS_NW_SET'] = ""
            org_data['WC_LIST'] = self.ctlr_dict[org_data['NODE']]
            org_data['WCDICT'] = {}
...
def main():
    print((datetime.now()))
    # cpe1 = VersaLib('VD1', topofile="Devices.csv")
    cpe2 = VersaLib('CPE26-HKG-SINGLE-CPE-HYBRID-IPC00190-IPV6', topofile="Devices.yml")
    cpe2.cross_login()
    time.sleep(60)
    cpe2.show_conf_bgp("LAN-EBGP")
    # cpe1.login()
    # cpe1.get_data_dict
    # cpe1.move_policy_rule("sample", "9999", "Default-Policy", "plcyrule_1", "first")
    output = cpe2.check_lan_route("1")
    print(output)
