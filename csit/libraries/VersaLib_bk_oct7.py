...
class VersaLib:
        ...
        if 'device_type' in self.__dict__:
            if self.device_type == 'versa_director':
                ...
                if ".csv" in self.topofile:
                    ...
                    for i, k in list(self.vdcsv_dict['VD1'].items()):
    ...
    def cpe_onboard_call(self):
        ...
        self.main_logger.debug(
            cpe_shell_login.send_command_expect(self.password, expect_string='CLI now allowed', strip_prompt=False,
                                                strip_command=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('cli', expect_string='>', strip_prompt=False, strip_command=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('request erase running-config', expect_string='yes', strip_prompt=False,
                                                strip_command=False))
        self.main_logger.debug(
            cpe_shell_login.send_command_expect('yes', expect_string='\$|#', strip_prompt=False, strip_command=False))
    ...
    # def show_conf_bgp_routing(self):
    #    cmd = "show configuration | display set | match routing-peer-policy"
    #    output = self.nc.send_command_expect(cmd, expect_string=">", strip_prompt=False, strip_command=False)
    #    print output
    #    return output

    def get_bgp_redis(self, Customer_ORG_Name, loopback_ip):
        # show route table l3vpn.ipv4.unicast routing-instance IPC00190-Control-VR receive-protocol bgp 123.1.2.3/32 | nomore
    ...
    def Create_Node_Data(self, node_dev, node_type, org_name="", org_id="", wan=""):
        ...
        for idx, row in csv_data_read.iterrows():
            ...
            if node_type == "WC":
                ...
                if node_dev in RR_Clients:
                    node_device_data["RR_CLIENTS"] = RR_Clients[node_dev]
                else:
                    for k, v in list(RR_Clients.items()):
    ...
    def config_bgp_redis(self, Device_name, Customer_ORG_Name, Customer_ORG_ID, sdwan_policy_name, lan_number,
                         redis_policy_name, community_id, **kwargs):
        temp_dict = {}
        if kwargs is not None:
            for k, v in kwargs.items():
                temp_dict[k] = v
...
def main():
    ...
    cpe2 = VersaLib('CPE11-HKG-HYBRD-IPC00190', topofile="Devices.csv")
    cpe2.cross_login()
    # cpe1.login()
    # cpe1.get_data_dict
