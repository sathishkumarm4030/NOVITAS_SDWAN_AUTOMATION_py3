...
def get_vd_details():
    global cpe_list
    ip = eval(input("Enter Versa Director IP address:\n"))
    print(("Versa director IP:" + ip))
    ldap_user = eval(input("Enter LDAP Username for making SSH connection to VD:\n"))
    print(("Versa director Username:" + ldap_user))
    ldap_passwd = getpass.getpass("Enter LDAP Password:\n")
    user = eval(input("Enter Username for making REST actions to Versa Director :\n"))
    print(("Versa director Username:" + user))
    passwd = getpass.getpass("Enter REST Password:\n")
    cpe_user = eval(input("Enter Versa CPE Username:\n"))
...
def DO_config_in_VD_For_CPEs():
    ...
    input("Edit " + VD1.curr_file_dir + "/Topology/" + VD1.cpe_list_file_name + " & Press enter to continue")
    VD1.vdnc = VD1.login()
    # VD1.vdnc = "1234"
