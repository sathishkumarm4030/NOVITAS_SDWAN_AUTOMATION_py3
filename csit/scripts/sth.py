...
class StcPython(object):
    ...
    def help(self, topic=''):
        ...
        if topic == 'commands':
            allCommands = []
            for commandHelp in list(StcIntPythonHelp.HELP_INFO.values()):
    ...
    def _packKeyVal(svec, hash):
        for key, val in list(hash.items()):
            svec.append('-' + str(key))
            if type(val) is list:
...
def _pydict2tclstr(handle_ret_py, dict_ret_py, key_str):
    ...
    if re.match('^py_port', handle_ret_py, flags=re.I) is not None:
        ...
        for (key, value) in list(dict_ret_py.items()):
...
def _hlt_params_conv(**arg):
    ...
    for key in list(arg.keys()):
        ...
        elif sys.version_info < (3,0) and type(arg[key]) is str:
...
def connect(**arg):
    ...
    #print "HLPY: connect:",
    port_names = ''
    if 'my_return_handle' in list(arg.keys()):
    ...
    while(indexRet<index2):
        if "/" in str(ret[indexRet]):
                ...
                if ipKeyStr in list(chassis_port.keys()):
    ...
    if port_names:
        ...
        if 'connect' in list(GRetDict.keys()):
...
def cleanup_session(**arg):
    if 'my_port_handle' in list(arg.keys()) :
    ...
    cmdline = _hlt_params_conv(**arg)
    if "clean_logs" in list(arg.keys()) and int(arg["clean_logs"])==1:
...
def _process_cmd(cmd_name,**arg):
    ...
    if 'my_return_handle' in list(arg.keys()) or 'my_port_handle' in list(arg.keys()) or 'my_handle' in list(arg.keys()) :
...
def _process_handles(cmd_name,**arg):
    ...
    if device_names :
        names_dict = {}
        if cmd_name in list(GRetDict.keys()):
